...
- **4.1 Neural networks, market setup and π_M.** Feed-forward networks, families NN_{M,d0,d1} with exhaustion / zero network / zero-weight embedding; Hornik's universal approximation theorem (density in L^p(μ) for every finite measure μ) with the remark that Hornik is already a general-measure statement — finiteness of Ω is used elsewhere. Semi-recurrent parametrization δ_k = F_{θ_k}(I_0, ..., I_k, δ_{k−1}); unconstrained rewriting via H∘δ (Lemma 4.2 of Bühler et al.); the approximate problem π_M(X). Ends with Lemma 4.1, the monotone sandwich for π_M (π_M ↓ and π_M ≥ π; no finiteness, no (A1)/(A6)), which the finite-case proof will use immediately.
- **4.2 The finite-Ω approximation theorem and what breaks.** Statement and full proof of Proposition 4.3 of Bühler et al. (π_M(X) ↓ π(X)), invoking the monotone sandwich, followed by a visible discussion of where finiteness enters — exactly three failure points:
...
- **4.3 Repair under the Chapter 3 hypotheses.** Lead-in (the gaps are (F1) and (F3); Chapter 3 already supplies (A1) and (A6), which give a constant envelope and survive truncation; (A2)–(A5) return only for attainment) → Lemma 4.4 constant-envelope upper semicontinuity of OCEs (repairs (F3)) → Lemma 4.6 truncation / value-density of bounded strategies under (A1) and (A6) (repairs (F1)) → Theorem 4.8: under the standing setting, OCE, (A1) and (A6) alone, π_M(−Z) → π(−Z) monotonically from above → Corollary 4.10, the thesis in one line: under (A1)–(A6), neural network strategies are ε-optimal for an *attained* optimum. Optional remark on the weakening (A1′); no sharpness counterexample.
...
- **Each folder contains multiple artifacts:** drafts, structure files, files containing math / lemmas / experiments (e.g. `extension_of_approximation_on_general.tex`, `prop4_3_finite.tex`), plus a `bib.md` listing all sources needed for that part.
- **Standalone compilable artifacts (all `.tex` artifacts).** Every mathematical / draft `.tex` artifact must be a complete, self-contained document with its own preamble (`\documentclass`, packages, theorem environments, `\begin{document}` … `\end{document}`), so it builds to a PDF on its own (e.g. via LaTeX Workshop on save). Fragments without a preamble are not allowed. When the thesis is assembled at the end, strip each artifact's standalone preamble and merge the body only; keep `% draft: …` comments for old numbering.
- **Faked cross-references in artifacts.** Artifacts must compile without depending on lemmas/equations/results defined only in other files. Do **not** use `\ref` / `\eqref` / `\cite` toward targets that are not defined in the same artifact (that produces `??` or forces scaffolding lemmas). Instead: hardcode a plausible number or citation text in the visible body (e.g. `Lemma~4.4`, `(4.1)`, `B\"uhler et al.\ \cite{buehler}` only if the bib entry is local — otherwise write the citation in plain text). Directly above each faked reference, put a LaTeX comment (invisible in the PDF) of the form `% FAKE REF: "Lemma~4.4" -> replace by \ref{lem:monotone}` plus where the real target lives (section / other artifact / draft label). On final merge, replace every faked reference by the real `\ref` / `\eqref` / `\cite`. Example of the convention: `Chapter 4/prop4_3_finite.tex`.
