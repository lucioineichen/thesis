...
Provisional numbering below follows the intended labels (Lemma~4.1, Prop.~4.3, Prop.~4.7, Lemmas~4.4/~4.6, Thm~4.8, Cor~4.10). Renumber cleanly when assembling if needed; keep Cor~4.10 as the thesis punchline.
...
- State the chapter plan: finite-$\Omega$ result of Bühler et al.\ $\to$ where finiteness is used $\to$ what fails on general $\Omega$ $\to$ repair under (A1),(A6) $\to$ algorithm $\to$ strategies need not converge.
- Scope: OCE only
...
- Statement and full proof: $\pi_M(X)\downarrow\pi(X)$ on finite $\Omega$.
...
One continuous section: lead-in $\to$ tools $\to$ theorem $\to$ punchline corollary. Do not split tools and theorem into separate top-level sections.
...
- Announce the plan: prepare three lemmas, prove Theorem~4.8, then combine with Chapter~3.

### Lemma 4.1 — constant-envelope upper semicontinuity of OCEs
...
### Lemma 4.4 — monotone sandwich for $\pi_M$
...
- Proof outline: near-optimal target $\to$ truncate (Lemma~4.6) $\to$ Hornik in $L^1$ $\to$ a.s.\ subsequence $\to$ continuity of $H_k$, usc of costs $\to$ Lemma~4.1.
...
- [ ] Prop.~4.3 proved in full; finiteness discussion visible as its own block (F1)–(F3) only; gains/costs not an F
