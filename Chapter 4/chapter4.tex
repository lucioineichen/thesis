...
B\"uhler et al.\ \cite[Definition~2]{buehler} define a feed-forward
neural network with activation function $\sigma\colon\R\to\R$ as a map
\[
  F=W_L\circ F_{L-1}\circ\dots\circ F_1\colon\R^{d_0}\to\R^{d_1}
\]
with $F_j=\sigma\circ W_j$ for affine maps $W_j$, where $\sigma$ acts
componentwise. We denote by $\NNet_{\infty,d_0,d_1}$ the set of all
such networks and fix, as in \cite[Section~4.1]{buehler}, an increasing
exhaustion $\NNet_{M,d_0,d_1}\subseteq\NNet_{M+1,d_0,d_1}$ with
...
\noindent
B\"uhler et al.\ prove the following on a finite probability space \cite[Proposition~4.3]{buehler}. In
the following proposition, we restate it and flag each use of finiteness of $\Omega$ as (F1) to (F3);
these are exactly the points that need attention on a general
probability space.
...
$\delta\in\HH$). From now on we restrict attention to optimal certainty equivalent (OCE) risk measures.

% FAKE REF: OCE definition -> Chapter 2, Section 2.6.
The first lemma repairs (F3); the failure of the risk measure to be
upper semicontinuous along almost surely convergent sequences. Recall from Chapter~2 that the OCE risk measure associated with a convex nondecreasing loss
function $\ell:\R\to\R$ is
\[
...
\begin{proof}
  ...
  By (A1) and (A6),
  \[
    Y_j=-Z+W(\eta^{(j)})\ge-\|Z\|_\infty\as
  \]
  for every $j$. Lemma~\ref{lem:usc} with $B=\|Z\|_\infty$ gives
  \[
    \limsup_{j\to\infty}\rho_\ell(Y_j)\le\rho_\ell(Y).
  \]
  Choose $j_\varepsilon\in\N$ large enough that
  $\rho_\ell(Y_{j_\varepsilon})\le\rho_\ell(Y)+\varepsilon$, and set
  $\delta':=\delta^{(j_\varepsilon)}$.
\end{proof}
...
and the resulting finite-dimensional problem is minimized by the Adam
algorithm, as in B\"uhler et al.\ \cite[Section~5]{buehler}. For
...
$\pi(-Z)$, and near-minimizers are $\varepsilon$-optimal. Nothing of
this kind holds for the strategies themselves. Absence of strict
convexity of $\rho$ (so optimal terminal P\&Ls need not be unique),
redundancy in the market (so distinct strategies may share the same
gain) and lack of compactness in the strategy space (so
near-minimizing sequences can wander even when the optimizer is unique)
can each prevent a sequence of network near-minimizers
$\delta^M\in\HH_M$ from converging in any reasonable topology.

% draft: Example (CVaR admits multiple optimal P&Ls);
% same as ex:cvar-nonunique in extension_with_A1_to_A6.tex.
\begin{example}\label{ex:cvar-nonunique}
  Average value at risk is a popular not strictly convex risk measure. Any P\&Ls with the same mean above the VaR threshold give the same risk; allowing for infinitely many optimal terminal P\&Ls.
\end{example}

% FAKE REF: the (A5) illustration in Chapter 3 (digital option via
%   call-call butterfly with positions exploding).
\begin{example}\label{ex:digital-explosion}
  Even convergence of wealth does not force strategy convergence. The
  remark on (A5) in Chapter~3 already shows this. Assume zero costs,
  tradability in options $A=(S_T-K)^+$ and $B_n=(S_T-(K+1/n))^+$, and feasability of the trading strategy
  \[
    \delta_n:=nA-nB_n.
  \]
  Then, the terminal P\&L of $\delta_n$ converges pointwise to the digital
  payoff $\mathbf{1}_{\{S_T>K\}}$. On the other hand, the position of holding the digital option is finite while the dynamic strategy explodes:
  $\|\delta_n\|_{L^p}\to\infty$.
\end{example}
